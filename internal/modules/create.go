...
func (m *ModuleController) createModuleTool() mcp.Tool {
	return mcp.NewTool("create_module",
		mcp.WithDescription("Create new Module. Before calling this tool, make sure to call get_template_schema to validate values for the given template"),
...
func (m *ModuleController) createModule(_ context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err := m.k8sClient.CreateModule(module); err != nil {
		return nil, fmt.Errorf("failed to update module: %w", err)
	}

	return mcp.NewToolResultText(fmt.Sprintf("Module %s created successfully", moduleName)), nil
}
